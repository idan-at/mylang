...
describe("Lexer", () => {
  describe("keywords", () => {
    ...
    it("should properly tokenize elsif keyword", () => {
      const code = `elsif `;

      expect(code).toTokenizeTo([new Token(ELSIF, "elsif ", 1, 1)]);
    });

    it("should properly tokenize else keyword", () => {
      const code = `else `;
  ...
  it("throws when no match is found", () => {
    ...
    expect(code).toFailTokenizing("unexpected token '#' (1:1)");
  });
});
