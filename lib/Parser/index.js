...
class Parser {
  ...
  _literal(name) {
    ...
    switch (next.symbol) {
      ...
      default:
        throw new SyntaxError(`invalid syntax '${next.text}' ${next.posString}`)
    }
  }
  ...
  _function(name = 'anonymous') {
    ...
    switch (next.symbol) {
      ...
      default:
        return new FunctionExpression(name, [this._expression()], args, varargs)
    }
  }
      ...
      switch (arg.symbol) {
        case IDENTIFIER:
          if (args.includes(arg.text)) {
            throw new ParserError(`double argument error: '${arg.text}' already exists ${arg.posString}`)
          }
          args.push(arg.text)
          break
        case VARARGS:
          ...
          if (!noMoreArgs()) {
            throw new ParserError(`'${arg.text}' must be used as the last argument ${arg.posString}`)
          }
          varargs = true
          args.push(arg.text)
          break
        default:
          throw new ParserError(`expected '${arg.text}' to be an identifier ${arg.posString}`)
      }
    }
