...
class VM {
  ...
  _functionContext(f, args, symbols) {
    ...
    if (f.varargs) {
      ...
      newSymbols.set(f.argAt(restArgIndex).name.substr(1), rest);
      args.forEach((arg, idx) => {
        if (idx < restArgIndex) {
          newSymbols.set(f.argAt(idx).name, this._evalExp(arg, symbols));
        } else {
          try {
      ...
      fArgs.forEach((arg, idx) => {
        if (args[idx]) {
          newSymbols.set(arg.name, this._evalExp(args[idx], symbols));
        } else {
          newSymbols.set(arg.name, this._evalExp(arg.value, symbols));
        }
      });
    }
  ...
  _validFunctionCall(f, args) {
    ...
    return args.length >= f.args.filter(arg => !arg.hasDefaultValue).length;
  }
}
