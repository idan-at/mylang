...
const { FunctionExpression, FunctionCall } = require('../Parser/AST')
const {
  isNil,
  isTrue,
  isFalse,
...
function createGlobalFunction(name, args, fn, varargs = false) {
  globals.set(name, new FunctionExpression(name, symbols => {
    const fArgs = args.map(arg => arg.startsWith('@') ? arg.substr(1) : arg)
...
// Prints
createGlobalFunction('println', ['item'], (_, item) => console.log(format(item)))

// Math
createGlobalFunction('+', ['a', 'b', '@rest'], (_, a, b, rest) => {
...
createGlobalFunction('and', ['a', 'b', '@rest'], (_, a, b, rest) => {
...
createGlobalFunction('or', ['a', 'b', '@rest'], (_, a, b, rest) => {
  validateType({ value: a, name: 'a' }, [{ fn: isBoolean, typeName: BOOLEAN }])
  validateType({ value: b, name: 'b' }, [{ fn: isBoolean, typeName: BOOLEAN }])
...
createGlobalFunction('if', ['condition', 'then', 'else'], (_, condition, then, else_) => {
...
// Data structures
// List
createGlobalFunction('list', ['@rest'], (_, rest) => {
  return [...rest]
}, true)

// Set
createGlobalFunction('set', ['@rest'], (_, rest) => {
  return new Set(rest)
}, true)

// Dictionary
createGlobalFunction('dict', ['@rest'], (_, rest) => {
  if (rest.length % 2 !== 0) {
    rest.push(null)
  }
